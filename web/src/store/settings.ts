...
export type OcrEngineId = 'tesseract' | 'claude' | 'google';
...
const emptyUsage = (): UsageCounters => ({ tesseract: 0, claude: 0, google: 0 });
...
export const useSettings = create<SettingsState>()(
  persist(
    (set, get) => ({
      ...
      setGoogleKey: (v) => set({ googleKey: v.trim() }),
...
export const USAGE_REFERENCE: Record<OcrEngineId, number | null> = {
  tesseract: null, // unlimited / local — bar shows raw count
